...
//input coordinates
void Rectangle::feedSensorData()
{
    int t = 0 ;
    for(int i=0; i<8; i=i+2)
    {
        cout << "Please enter x-ordinate of pt." << ++t << ": ";
        cin >> vertices[i] ;
        cout << "Please enter y-ordinate of pt." << t << ": ";
        cin >> vertices[i+1] ;
    }
}
...
string Rectangle::toString()
{
    ...
    for(int i=0; i<8; i=i+2)
    {
        temp = "(" + to_string(vertices[i]) + "," + to_string(vertices[i+1]) + ") ";
        tempString += temp;
    }
    return tempString;
}
...
int Rectangle::greatest(int a, int b, int c, int d)  
{  
    if (a > b) {  
        if (a > c) {  
            if (a > d) {  
...
void Rectangle::displayPointsOnPerimeter()
{
    int max_x = greatest(vertices[0],vertices[2],vertices[4],vertices[6]);
    int max_y = greatest(vertices[1],vertices[3],vertices[5],vertices[7]);
    bool found=false;
